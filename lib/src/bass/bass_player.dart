...
class BassPlayer {
  ...
  BassPlayer() {
    final dyLibDir = path.join(
      path.dirname(Platform.resolvedExecutable),
      "BASS"
    );
    _dyLib = ffi.DynamicLibrary.open(path.join(dyLibDir, 'libbass.so'));
    _bass = Bass(_dyLib);
    ...
    for (final plugin in BASS_PLUGINS) {
      final pluginPathP = path.join(dyLibDir, plugin).toNativeUtf16() as ffi.Pointer<ffi.Char>;
